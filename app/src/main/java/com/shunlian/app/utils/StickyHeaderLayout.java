...
import com.shunlian.app.adapter.GroupedRecyclerViewAdapter;
...
public class StickyHeaderLayout extends FrameLayout {
    ...
    private final SparseArray<BaseRecyclerAdapter.BaseRecyclerViewHolder> mStickyViews = new SparseArray<>();
        ...
        if (adapter instanceof GroupedRecyclerViewAdapter) {
            ...
            if (mCurrentStickyGroup != groupPosition) {
                ...
                if (groupHeaderPosition != -1) {
                    ...
                    BaseRecyclerAdapter.BaseRecyclerViewHolder holder = recycleStickyView(viewType);
     ...
     * @return
     */
    private BaseRecyclerAdapter.BaseRecyclerViewHolder recycleStickyView(int viewType) {
        if (mStickyLayout.getChildCount() > 0) {
            View view = mStickyLayout.getChildAt(0);
            int type = (int) view.getTag(VIEW_TAG_TYPE);
            if (type == viewType) {
                return (BaseRecyclerAdapter.BaseRecyclerViewHolder) view.getTag(VIEW_TAG_HOLDER);
            } else {
                recycle();
            }
        }
    ...
    private void recycle() {
        if (mStickyLayout.getChildCount() > 0) {
            ...
            mStickyViews.put((int) (view.getTag(VIEW_TAG_TYPE)),
                    (BaseRecyclerAdapter.BaseRecyclerViewHolder) (view.getTag(VIEW_TAG_HOLDER)));
     ...
     * @return
     */
    private BaseRecyclerAdapter.BaseRecyclerViewHolder getStickyViewByType(int viewType) {
